...
$db = pg_connect("host=localhost port=5432 dbname=WebOV user=postgres password=masterkey");

 $gen = $_GET['id'];
 
  
$result = pg_query($db,"SELECT * from students where student_id ='$gen'" );
...
while($row = pg_fetch_array($result)) {
 ...
 echo  "<td>" . $password . "</td>";  
  ...
  //echo "<td><a href='print.php?id=".$row['idstudent']."'>print</a></td>";
...
// $result1= pg_query($db,SELECT * from student_details");
 // $row1 = pg_fetch_array($result1);

//get ID from cookie that set a while in login form
$userID = $_COOKIE["USERID"];
//create query for expiration
$expiration = "select CURRENT_DATE + 5 date";
$results = pg_query($expiration);
//fetch data from created query
$arr = pg_fetch_array ($results);

$result = pg_query($db,"insert into voters_passwords(vpassword,expirydate,account_id) values('" .$password."' , '" . $arr['date'] . "','" .$userID ."')");
...
<html>
 <head>
 <style type="text/css" medi="print">
 @media print
 {
 #none-printable {display: none;}
 #printable {
 display:block;
 width:100%;
 height:100%;
